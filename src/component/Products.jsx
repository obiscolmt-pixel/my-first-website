...
const GADGET_CATEGORIES = [
  ...
  },
  {
    ...
    name: "Tablets",
    emoji: "📟",
    image: "https://images.unsplash.com/photo-1544244015-0df4b3ffc6b0?w=400&q=80&fit=crop",
    bg: "#3a1a1a",
  },
  {
    id: "accessories",
    ...
    <div className="min-h-screen" id="products">
                ...
                <div className="border-t pt-4">
                  ...
                  {showReviewForm && (
                    <div className="bg-orange-50 rounded-xl p-3 mb-3 border border-orange-100">
                      ...
                      <textarea value={userComment} onChange={(e) => setUserComment(e.target.value)} placeholder="Share your experience with this product..." rows={3} className="w-full border rounded-xl px-3 py-2 text-xs focus:outline-none focus:border-orange-500 resize-none" />
      ...
      <div className="relative overflow-hidden h-[300px] sm:h-[400px]">
        ...
        <div className="absolute inset-0 bg-black/60 flex items-center px-6 sm:px-16">
          <div>
            <p className="text-orange-500 uppercase tracking-[0.3em] text-xs font-semibold mb-2">Obisco Store</p>
            <h1 className="text-4xl sm:text-6xl font-black text-white leading-tight">
              Gadgets & <span className="text-orange-500">Tech</span>
            </h1>
            <p className="text-gray-300 text-sm mt-3 max-w-md">
              Phones, Laptops, Tablets, Accessories — top tech delivered across Nigeria.
            </p>
            <button
              onClick={() => document.getElementById("gadgets-products")?.scrollIntoView({ behavior: "smooth" })}
      ...
      <div className="max-w-[1640px] mx-auto px-4 py-10">
        ...
        <div className="text-center mb-8">
          <p className="text-orange-500 uppercase tracking-[0.3em] text-xs font-semibold mb-2">Browse By Type</p>
          ...
          {GADGET_CATEGORIES.map((cat) => (
            <div
              ...
              onClick={() => {
                ...
                document.getElementById("gadgets-products")?.scrollIntoView({ behavior: "smooth" });
              }}
              className="group relative overflow-hidden rounded-2xl h-[160px] cursor-pointer hover:-translate-y-1 transition-all duration-300 hover:shadow-[0_12px_30px_rgba(249,115,22,0.25)]"
            >
              <img src={cat.image} alt={cat.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
              <div className="absolute inset-0 bg-gradient-to-t from-black/75 via-black/20 to-transparent" />
              <div className="absolute bottom-0 left-0 right-0 p-3 text-center">
                <h3 className="text-white font-bold text-sm">{cat.emoji} {cat.name}</h3>
                <p className="text-orange-400 text-xs mt-0.5 opacity-0 group-hover:opacity-100 transition-opacity duration-300">Shop Now →</p>
              </div>
        ...
        <div id="gadgets-products">
          <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
            <h2 className="font-black text-2xl sm:text-3xl bg-gradient-to-r from-orange-600 to-yellow-400 bg-clip-text text-transparent">
              {activeType === "All" ? "All Gadgets" : activeType.charAt(0).toUpperCase() + activeType.slice(1)}
            </h2>
            ...
            {displayedProducts.map((item) => (
              <div key={item._id} className="shadow-md rounded-xl hover:scale-105 duration-300 overflow-hidden bg-white cursor-pointer">
                ...
                <div className="flex items-center gap-1 px-2 pb-1">
                  <div className="flex">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <span key={star} className={`text-xs ${star <= Math.round(item.avgRating || 0) ? "text-yellow-400" : "text-gray-200"}`}>★</span>
                    ))}
                  </div>
                  <span className="text-[10px] text-gray-400">
                    {item.avgRating ? `${item.avgRating} (${item.totalReviews})` : "No reviews"}
                  </span>
                </div>
          ...
          {displayedProducts.length === 0 && !loading && (
            <div className="text-center py-16">
              ...
              <p className="text-gray-400 text-base sm:text-lg">
                {searchQuery ? `No products found for "${searchQuery}"` : "No gadgets available yet"}
              </p>
              <button
                onClick={() => { setActiveType("All"); setActivePrice(null); setProducts(allProducts); }}
        ...
        {recentlyViewed.length > 0 && (
          <div className="mt-12 mb-6">
            <div className="flex items-center gap-3 mb-4">
              <h2 className="font-black text-xl text-gray-800">👁️ Recently Viewed</h2>
              <button
                onClick={() => { setRecentlyViewed([]); localStorage.removeItem("obisco_recently_viewed"); }}
              ...
              {recentlyViewed.map((item) => (
                ...
                >
                  ...
                  <div className="p-2">
                    ...
                    <p className="text-gray-400 text-[10px] capitalize mt-0.5">{item.category}</p>
                  </div>
                </div>
              ))}
            </div>
