...
import { fashionProducts, fashionCategories } from "../data/data.js";
...
const FashionPage = ({ addToCart, addToWishlist, isWishlisted, searchQuery }) => {
  ...
  const displayed = fashionProducts.filter((p) => {
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-12">
          {fashionCategories.map((cat) => (
            <div
              ...
              className="group relative overflow-hidden rounded-2xl cursor-pointer h-[160px] hover:-translate-y-1 transition-all duration-300 hover:shadow-[0_12px_30px_rgba(249,115,22,0.25)]"
            >
              <img
                src={cat.image}
                alt={cat.name}
        ...
        <div id="fashion-products">
          <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
            ...
            <div className="flex flex-wrap gap-2">
              {["all", ...fashionCategories.map((c) => c.name.toLowerCase())].map((cat) => (
                <button
                  key={cat}
