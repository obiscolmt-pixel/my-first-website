...
export default function VTUPage({ onClose }) {
  ...
  const handleAirtime = async () => {
    ...
    if (vtuPaymentMethod === "wallet") {
      ...
      try {
        ...
        if (data.code === "000" || data.response_description === "TRANSACTION SUCCESSFUL") {
          ...
          setError(data.response_description || "Transaction failed. Contact support.");
        }
    ...
    if (!email) {
      setShowEmailInput(true); setPendingAction("airtime"); setPendingAmount(amount); return;
    }
  ...
  const handleData = async () => {
    ...
    const email = getUserEmail();
    if (!email) {
      setShowEmailInput(true); setPendingAction("data"); setPendingAmount(amount); return;
    }
    setLoading(true); setError("");
    initiatePaystackPayment(amount, email, async () => {
      try {
        const res = await fetch(`${API_BASE}/api/vtu/data`, {
          ...
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            network: dataNetwork, phone: dataPhone,
            variationCode: selectedPlan, amount: plan?.variation_amount,
          }),
        });
        const data = await res.json();
        if (data.code === "000" || data.response_description === "TRANSACTION SUCCESSFUL") {
  ...
  const handleElectricity = async () => {
    ...
    const email = getUserEmail();
    if (!email) {
      setShowEmailInput(true); setPendingAction("electricity"); setPendingAmount(amount); return;
    }
    setLoading(true); setError("");
    initiatePaystackPayment(amount, email, async () => {
      try {
  ...
  const handleCable = async () => {
    ...
    const email = getUserEmail();
    if (!email) {
      setShowEmailInput(true); setPendingAction("cable"); setPendingAmount(amount); return;
    }
    setLoading(true); setError("");
    initiatePaystackPayment(amount, email, async () => {
      try {
        const res = await fetch(`${API_BASE}/api/vtu/cable`, {
          ...
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            provider: cableProvider, smartCardNumber: smartCard,
            variationCode: selectedCablePlan, amount: plan?.variation_amount, phone: cablePhone,
          }),
        });
        const data = await res.json();
        if (data.code === "000" || data.response_description === "TRANSACTION SUCCESSFUL") {
  ...
  const handleEmailSubmit = () => {
    ...
    initiatePaystackPayment(pendingAmount, customerEmail, async () => {
      if (pendingAction === "airtime") {
        ...
        const data = await res.json();
        if (data.code === "000" || data.response_description === "TRANSACTION SUCCESSFUL") {
          setResult({ success: true, message: `✅ ₦${Number(airtimeAmount).toLocaleString()} airtime sent to ${airtimePhone}!`, data });
          setAirtimePhone(""); setAirtimeAmount(""); setAirtimeNetwork("");
      ...
      if (pendingAction === "data") {
        ...
        const data = await res.json();
        if (data.code === "000" || data.response_description === "TRANSACTION SUCCESSFUL") {
          setResult({ success: true, message: `✅ ${plan?.name} data sent to ${dataPhone}!`, data });
          setDataPhone(""); setDataNetwork(""); setSelectedPlan("");
        } else { setError(data.response_description || "Transaction failed."); }
      }
      ...
      if (pendingAction === "cable") {
        ...
        const data = await res.json();
        if (data.code === "000" || data.response_description === "TRANSACTION SUCCESSFUL") {
          setResult({ success: true, message: `✅ ${plan?.name} subscription activated!`, data });
        } else { setError(data.response_description || "Transaction failed."); }
      }
      setLoading(false);
    });
    ...
    <div className="space-y-2">
      ...
      <div className="flex gap-2">
        <button
          ...
          className={`flex-1 py-2.5 rounded-xl text-xs font-bold border-2 transition flex flex-col items-center gap-0.5 ${
            vtuPaymentMethod === "paystack"
              ? "bg-orange-500 text-white border-orange-500"
              : "border-gray-200 text-gray-600 hover:border-orange-300 bg-white"
          }`}
        >
          <span>💳 Card</span>
          <span className={`text-xs font-normal ${vtuPaymentMethod === "paystack" ? "text-orange-100" : "text-gray-400"}`}>
            Paystack
          </span>
        </button>
        <button
        ...
        >
          <span>💰 Wallet</span>
          <span className={`text-xs font-normal ${vtuPaymentMethod === "wallet" ? "text-orange-100" : "text-gray-500 font-semibold"}`}>
            ₦{walletBalance.toLocaleString()}
          </span>
        </button>
      </div>
      {vtuPaymentMethod === "wallet" && amount > 0 && (
        <div className={`rounded-xl p-2.5 border text-xs font-medium ${
          ...
          {walletBalance >= amount
            ...
            : `❌ You need ₦${(amount - walletBalance).toLocaleString()} more — fund your wallet first`}
        </div>
      )}
    </div>
  );
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
                  ...
                  {NETWORKS.map((n) => (
                    <button
                      ...
                      className={`p-2 rounded-xl border-2 text-center transition-all ${
                        airtimeNetwork === n.id ? "border-orange-500 shadow-md" : "border-gray-200 hover:border-orange-300"
                      }`}
                    >
          ...
          {activeTab === "data" && (
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-4 gap-2">
                  {DATA_NETWORKS.map((n) => (
                    <button
                      ...
                      className={`p-2 rounded-xl border-2 text-center transition-all ${
                        dataNetwork === n.id ? "border-orange-500 shadow-md" : "border-gray-200 hover:border-orange-300"
                      }`}
                    >
                      <div className={`w-9 h-9 rounded-full ${n.bg} mx-auto flex items-center justify-center`}>
                        <span className={`text-xs font-black ${n.text}`}>{n.name[0]}</span>
              ...
              <PaymentToggle amount={Number(dataPlans.find(p => p.variation_code === selectedPlan)?.variation_amount) || 0} />
              ...
              <button
                ...
                className="w-full bg-black text-white py-3.5 rounded-xl font-bold hover:bg-gray-800 disabled:opacity-50 transition-colors text-sm"
              >
                {loading ? "⏳ Processing..." : `${vtuPaymentMethod === "wallet" ? "💰" : "💳"} Pay & Get Data`}
              </button>
            </div>
          )}
