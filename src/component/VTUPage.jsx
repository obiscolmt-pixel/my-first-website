...
const saveBeneficiary = (type, phone, network) => {
  ...
  const updated = [{ phone, network, savedAt: Date.now() }, ...filtered].slice(
    0,
    5,
  );
...
export default function VTUPage({ onClose }) {
  ...
  // Auto-fill last used airtime number
  useEffect(() => {
    if (screen === "airtime") {
      const valid = getValidSaved("airtime");
      if (valid.length > 0) {
        setAirtimePhone(valid[0].phone);
        setAirtimeNetwork(valid[0].network);
        setDetectedAirtimeNetwork(valid[0].network);
      }
    }
  }, [screen]);

  // Auto-fill last used data number
  useEffect(() => {
    if (screen === "data") {
      const valid = getValidSaved("data");
      if (valid.length > 0) {
        setDataPhone(valid[0].phone);
        setDataNetwork(valid[0].network);
        setDetectedDataNetwork(valid[0].network);
      }
    }
  }, [screen]);
  ...
  const verifyMeter = async () => {
    if (!disco || !meterNumber || verifying) return; // ← add verifying check here
    setVerifying(true);
    setMeterName("");
    setError("");
    try {
      ...
      console.log("Verify response:", data); // ← add this to see response in browser console
  ...
  const PageHeader = ({ title, onBack }) => (
    <div className="flex items-center gap-4 p-5 border-b bg-white sticky top-0 z-10 flex-shrink-0">
      <button
        onClick={onBack}
        className="w-10 h-10 flex items-center justify-center text-gray-600 hover:text-orange-500 transition"
      >
        ...
        <div className="flex flex-col h-full">
          <div className="bg-orange-500 px-5 pt-12 pb-8 flex-shrink-0">
            <div className="flex justify-between items-center mb-6">
              <button
                onClick={onClose}
                className="w-10 h-10 flex items-center justify-center text-white hover:text-orange-200 transition"
              >
                <span style={{ fontSize: 32, lineHeight: 1, fontWeight: 300 }}>
                  ‹
                </span>
              </button>
                      ...
                      {["Hot", "Daily", "Weekly", "Monthly", "Other"].map(
                        (f) => (
                          <button
                            ...
                            className={`px-4 py-2 rounded-full text-xs font-bold whitespace-nowrap flex-shrink-0 transition-all ${planFilter === f ? (f === "Hot" ? "bg-red-500 text-white" : "bg-orange-500 text-white") : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}
                          >
                            {f === "Hot" ? "🔥 Hot" : f}
                          </button>
                        ),
                            ...
                            {filteredPlans.map((plan) => {
                              ...
                              return (
                                <button
                                  ...
                                  className={`p-3 rounded-2xl border-2 text-center transition-all relative ${isSelected ? "border-orange-500 bg-orange-50 shadow-md" : "border-gray-200 bg-gray-50 hover:border-orange-300 hover:bg-orange-50/50"}`}
                                >
                                  {isSelected && (
