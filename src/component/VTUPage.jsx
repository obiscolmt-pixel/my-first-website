...
const detectNetwork = (phone) => {
  ...
  const mtn = [
    ...
    "0903","0906","0913","0916","0704","0803","0806","0810",
...
// ── Saved beneficiaries ──
const getSaved = (type) => {
  try { return JSON.parse(localStorage.getItem(`obisco_beneficiaries_${type}`) || "[]"); }
  catch { return []; }
};
...
const saveBeneficiary = (type, phone, network, name = "") => {
  ...
  const updated = [{ phone, network, name, savedAt: Date.now() }, ...filtered].slice(0, 5);
...
const networkLabel = (id) => {
  const map = { mtn: "MTN", airtel: "Airtel", glo: "Glo", etisalat: "9mobile", "mtn-data": "MTN", "airtel-data": "Airtel", "glo-data": "Glo", "etisalat-data": "9mobile" };
  return map[id] || id;
};
...
const BeneficiaryList = ({ type, onSelect, onDelete }) => {
  const [saved, setSaved] = useState([]);
  useEffect(() => { setSaved(getSaved(type)); }, [type]);

  if (saved.length === 0) return null;

  return (
    <div>
      <p className="text-xs font-bold text-gray-400 mb-3 uppercase tracking-widest">Recent Numbers</p>
      <div className="flex gap-3 overflow-x-auto pb-2">
        {saved.map((b) => (
          <div key={b.phone} className="flex-shrink-0 relative">
            <button
              onClick={() => onSelect(b)}
              className="flex flex-col items-center gap-1 p-3 bg-gray-50 border border-gray-200 rounded-2xl w-20 hover:border-orange-300 hover:bg-orange-50 transition"
            >
              <div className={`w-10 h-10 rounded-full ${networkBg(b.network)} flex items-center justify-center`}>
                <span className={`text-xs font-black ${networkText(b.network)}`}>{networkLabel(b.network)[0]}</span>
              </div>
              <p className="text-xs font-bold text-gray-700 text-center leading-tight">{b.phone.slice(-4)}</p>
              <p className="text-xs text-gray-400">{networkLabel(b.network)}</p>
            </button>
            <button
              onClick={() => { deleteBeneficiary(type, b.phone); setSaved(getSaved(type)); onDelete && onDelete(); }}
              className="absolute -top-1 -right-1 w-5 h-5 bg-red-500 text-white rounded-full text-xs flex items-center justify-center font-bold leading-none"
            >×</button>
          </div>
        ))}
      </div>
...
export default function VTUPage({ onClose }) {
  ...
  const verifyMeter = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/vtu/verify`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ billersCode: meterNumber, serviceID: disco, type: meterType }) });
  ...
  const verifySmartCard = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/vtu/verify`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ billersCode: smartCard, serviceID: cableProvider }) });
      const data = await res.json();
  ...
  const handleAirtime = async () => {
    ...
    const doRequest = async () => {
      const res = await fetch(`${API_BASE}/api/vtu/airtime`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ network: airtimeNetwork, phone: airtimePhone, amount, ...(vtuPaymentMethod === "wallet" ? { userId: user._id || user.id, paymentMethod: "wallet" } : {}) }) });
      return res.json();
    };
  ...
  const handleData = async () => {
    ...
    const user = getUser();
    const doRequest = async () => {
      const res = await fetch(`${API_BASE}/api/vtu/data`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ network: dataNetwork, phone: dataPhone, variationCode: selectedPlan.variation_code, amount, ...(vtuPaymentMethod === "wallet" ? { userId: user._id || user.id, paymentMethod: "wallet" } : {}) }) });
      return res.json();
    };
    if (vtuPaymentMethod === "wallet") {
      if (!user?._id && !user?.id) return setError("Please log in to pay with wallet");
  ...
  const handleElectricity = async () => {
    ...
    if (vtuPaymentMethod === "wallet") {
      ...
      setLoading(true); setError("");
      try {
        const res = await fetch(`${API_BASE}/api/vtu/electricity`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ disco, meterNumber, meterType, amount, phone: elecPhone, userId: user._id || user.id, paymentMethod: "wallet" }) });
        const data = await res.json();
        if (data.code === "000" || data.response_description === "TRANSACTION SUCCESSFUL") {
  ...
  const handleCable = async () => {
    ...
    if (vtuPaymentMethod === "wallet") {
      ...
      setLoading(true); setError("");
      try {
        const res = await fetch(`${API_BASE}/api/vtu/cable`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ provider: cableProvider, smartCardNumber: smartCard, variationCode: selectedCablePlan, amount, phone: cablePhone, userId: user._id || user.id, paymentMethod: "wallet" }) });
        const data = await res.json();
        if (data.code === "000" || data.response_description === "TRANSACTION SUCCESSFUL") {
  ...
  const handleEmailSubmit = () => {
    ...
    initiatePaystackPayment(pendingAmount, customerEmail, async () => {
      if (pendingAction === "airtime") {
        ...
        const data = await res.json();
        if (data.code === "000" || data.response_description === "TRANSACTION SUCCESSFUL") { saveBeneficiary("airtime", airtimePhone, airtimeNetwork); setResult({ success: true, message: `✅ ₦${Number(airtimeAmount).toLocaleString()} airtime sent!`, data }); setBeneficiaryKey(k => k + 1); }
        else setError(data.response_description || "Transaction failed.");
      }
      if (pendingAction === "data" && selectedPlan) {
        ...
        const data = await res.json();
        if (data.code === "000" || data.response_description === "TRANSACTION SUCCESSFUL") { saveBeneficiary("data", dataPhone, dataNetwork); setResult({ success: true, message: `✅ ${selectedPlan.name} sent!`, data }); setBeneficiaryKey(k => k + 1); }
        else setError(data.response_description || "Transaction failed.");
      }
  ...
  const PlanBottomSheet = () => {
    ...
    return (
      <div className="fixed inset-0 bg-black/60 z-[200] flex items-end justify-center" onClick={() => setShowPlanSheet(false)}>
        <div className="bg-white w-full sm:max-w-lg rounded-t-3xl flex flex-col" style={{ maxHeight: "85vh" }} onClick={e => e.stopPropagation()}>
          <div className="p-5 border-b flex justify-between items-center flex-shrink-0">
            ...
            <button onClick={() => setShowPlanSheet(false)} className="w-9 h-9 bg-gray-100 rounded-full flex items-center justify-center text-gray-500 font-bold text-lg">✕</button>
          </div>
          <div className="flex gap-2 p-4 overflow-x-auto flex-shrink-0 border-b">
            {filters.map(f => (
              <button key={f} onClick={() => setPlanFilter(f)} className={`px-4 py-2 rounded-full text-xs font-bold whitespace-nowrap flex-shrink-0 transition-all ${planFilter === f ? f === "Hot" ? "bg-red-500 text-white" : "bg-orange-500 text-white" : "bg-gray-100 text-gray-600"}`}>
                {f === "Hot" ? "🔥 Hot" : f}
              </button>
            ))}
          </div>
          <div className="flex-1 overflow-y-auto p-4 space-y-2">
            {filteredPlans.length === 0 ? (
              <div className="text-center py-10 text-gray-400 text-sm">{planFilter === "Hot" ? "No hot plans for this network" : "No plans in this category"}</div>
            ) : filteredPlans.map(plan => {
              const isSelected = selectedPlan?.variation_code === plan.variation_code;
              return (
                <button key={plan.variation_code} onClick={() => { setSelectedPlan(plan); setShowPlanSheet(false); }} className={`w-full text-left p-4 rounded-2xl border-2 transition-all ${isSelected ? "border-orange-500 bg-orange-50" : "border-gray-100 bg-gray-50 hover:border-orange-200"}`}>
                  <div className="flex justify-between items-center gap-2">
                    <p className={`text-sm font-bold leading-tight flex-1 ${isSelected ? "text-orange-700" : "text-gray-800"}`}>{plan.name}</p>
  ...
  const SuccessCard = ({ message, transactionId }) => (
    <div className="flex flex-col items-center justify-center py-12 px-6 text-center">
      ...
      {transactionId && <p className="text-xs text-gray-400 mb-8">Transaction ID: {transactionId}</p>}
  ...
  const PageHeader = ({ title, onBack }) => (
    <div className="flex items-center gap-4 p-5 border-b bg-white sticky top-0 z-10 flex-shrink-0">
      <button onClick={onBack} className="w-10 h-10 bg-gray-100 rounded-full flex items-center justify-center text-gray-600 hover:bg-gray-200 transition font-bold text-lg">←</button>
        ...
        <div className="flex flex-col h-full">
          <div className="bg-orange-500 px-5 pt-12 pb-8 flex-shrink-0">
            <div className="flex justify-between items-center mb-6">
              <button onClick={onClose} className="w-10 h-10 bg-orange-400 rounded-full flex items-center justify-center text-white font-bold text-lg hover:bg-orange-600 transition">←</button>
      ...
      {screen === "airtime" && (
        <div className="flex flex-col h-full">
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-5">
            {result?.success ? <SuccessCard message={result.message} transactionId={result.data?.content?.transactions?.transactionId} /> : (
              <>
                <BeneficiaryList
                  key={beneficiaryKey}
                  type="airtime"
                  onSelect={(b) => { setAirtimePhone(b.phone); setAirtimeNetwork(b.network); setDetectedAirtimeNetwork(b.network); }}
                  onDelete={() => setBeneficiaryKey(k => k + 1)}
                />

                <div>
                  <label className="text-sm font-bold text-gray-700 mb-2 block">Phone Number</label>
                  <input type="tel" value={airtimePhone} onChange={e => handleAirtimePhoneChange(e.target.value)} placeholder="" className={inputClass} />
            ...
            {result?.success ? <SuccessCard message={result.message} transactionId={result.data?.content?.transactions?.transactionId} /> : (
              <>
                <BeneficiaryList
                  key={beneficiaryKey}
                  type="data"
                  onSelect={(b) => { setDataPhone(b.phone); setDataNetwork(b.network); setDetectedDataNetwork(b.network); setSelectedPlan(null); setPlanFilter("Hot"); }}
                  onDelete={() => setBeneficiaryKey(k => k + 1)}
                />

                <div>
                  <label className="text-sm font-bold text-gray-700 mb-2 block">Phone Number</label>
                  <input type="tel" value={dataPhone} onChange={e => handleDataPhoneChange(e.target.value)} placeholder="" className={inputClass} />
                ...
                <div>
                  <label className="text-sm font-bold text-gray-700 mb-2 block">Data Plan</label>
                  {!dataNetwork ? (
                    <div className="p-5 border-2 border-dashed border-gray-200 rounded-2xl text-center text-gray-400 text-sm">Select a network first</div>
                  ) : loadingPlans ? (
                    <div className="p-5 rounded-2xl text-center text-orange-500 text-sm font-bold">⏳ Loading plans...</div>
                  ) : (
                    <button onClick={() => setShowPlanSheet(true)} className={`w-full p-4 rounded-2xl border-2 text-left transition-all ${selectedPlan ? "border-orange-500 bg-orange-50" : "border-gray-200 bg-gray-50 hover:border-orange-300"}`}>
                      {selectedPlan ? (
                        <div className="flex justify-between items-center">
                          <div>
                            <p className="text-sm font-black text-orange-700 leading-tight">{selectedPlan.name}</p>
                            <p className="text-xs text-orange-400 mt-1">Tap to change plan</p>
                          </div>
                          <p className="text-lg font-black text-orange-600">₦{Number(selectedPlan.variation_amount).toLocaleString()}</p>
                        </div>
                      ) : (
                        <div className="flex justify-between items-center">
                          <p className="text-sm text-gray-400 font-medium">Tap to select a plan</p>
              ...
              <>
                ...
                <div>
                  ...
                  <div className="flex gap-3">
                    ...
                    <button onClick={verifyMeter} disabled={verifying || !disco || !meterNumber} className="bg-orange-500 text-white px-5 rounded-2xl text-sm font-bold disabled:opacity-50 hover:bg-orange-600 transition whitespace-nowrap">{verifying ? "..." : "Verify"}</button>
                  </div>
              ...
              <>
                ...
                <div>
                  ...
                  <div className="flex gap-3">
                    ...
                    <button onClick={verifySmartCard} disabled={verifying || !cableProvider || !smartCard} className="bg-orange-500 text-white px-5 rounded-2xl text-sm font-bold disabled:opacity-50 hover:bg-orange-600 transition whitespace-nowrap">{verifying ? "..." : "Verify"}</button>
                  </div>
