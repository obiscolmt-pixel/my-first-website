...
// ✅ Uses env variable for production, falls back to localhost for dev
  ...
  const verifySmartCard = async () => {
    ...
    try {
      ...
      if (data.content?.Customer_Name) {
        ...
        setError('Smart card not found. Please check and try again.');
      }
    } catch {
      setError('Verification failed. Try again.');
    } finally {
  ...
  const handleAirtime = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/vtu/airtime`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ network: airtimeNetwork, phone: airtimePhone, amount: airtimeAmount })
      });
      const data = await res.json();
      if (data.code === '000' || data.response_description === 'TRANSACTION SUCCESSFUL') {
        setResult({ success: true, message: `✅ ₦${Number(airtimeAmount).toLocaleString()} airtime sent to ${airtimePhone} successfully!`, data });
        setAirtimePhone(''); setAirtimeAmount(''); setAirtimeNetwork('');
      } else {
        setError(data.response_description || 'Transaction failed. Try again.');
      }
    } catch {
  ...
  const handleData = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/vtu/data`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ network: dataNetwork, phone: dataPhone, variationCode: selectedPlan, amount: plan?.variation_amount })
      });
      const data = await res.json();
      if (data.code === '000' || data.response_description === 'TRANSACTION SUCCESSFUL') {
        setResult({ success: true, message: `✅ ${plan?.name} data sent to ${dataPhone} successfully!`, data });
        setDataPhone(''); setDataNetwork(''); setSelectedPlan('');
      } else {
        setError(data.response_description || 'Transaction failed. Try again.');
      }
    } catch {
          ...
          {tabs.map(tab => (
            <button
              ...
              className={`flex-1 py-3 text-xs font-bold whitespace-nowrap transition-colors ${
                activeTab === tab.id
                  ? 'border-b-2 border-orange-500 text-orange-500 bg-white'
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="grid grid-cols-4 gap-2">
                  {NETWORKS.map(n => (
                    <button key={n.id} onClick={() => setAirtimeNetwork(n.id)}
                      className={`p-2 rounded-xl border-2 text-center transition-all ${
                        airtimeNetwork === n.id
                          ? 'border-orange-500 shadow-md'
                          : 'border-gray-200 hover:border-orange-300'
                      }`}>
                      <div className={`w-9 h-9 rounded-full ${n.bg} mx-auto flex items-center justify-center`}>
              ...
              <div>
                ...
                <input type="tel" value={airtimePhone}
              ...
              <div>
                ...
                <input type="number" value={airtimeAmount}
                  onChange={e => setAirtimeAmount(e.target.value)}
                  placeholder="Minimum ₦50" className={inputClass} />
                {/* ✅ Wrapping quick amounts */}
                <div className="flex flex-wrap gap-2 mt-2">
                  {quickAmounts.map(a => (
                    <button key={a} onClick={() => setAirtimeAmount(String(a))}
                      className={`px-3 py-1.5 rounded-lg text-xs font-semibold border transition-colors ${
                        airtimeAmount === String(a)
                          ? 'bg-orange-500 text-white border-orange-500'
                          : 'bg-gray-100 text-gray-700 border-gray-200 hover:border-orange-400 hover:text-orange-500'
                      }`}>
                      ₦{a.toLocaleString()}
                    </button>
                  ))}
              ...
              <button onClick={handleAirtime} disabled={loading}
                className="w-full bg-black text-white py-3.5 rounded-xl font-bold hover:bg-gray-800 active:bg-gray-900 disabled:opacity-50 transition-colors text-sm">
                {loading ? '⏳ Processing...' : `Buy Airtime${airtimeAmount ? ` — ₦${Number(airtimeAmount).toLocaleString()}` : ''}`}
              </button>
            </div>
          )}
          ...
          {activeTab === 'data' && (
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="grid grid-cols-4 gap-2">
                  {DATA_NETWORKS.map(n => (
                    <button key={n.id} onClick={() => setDataNetwork(n.id)}
                      className={`p-2 rounded-xl border-2 text-center transition-all ${
                        dataNetwork === n.id
                          ? 'border-orange-500 shadow-md'
                          : 'border-gray-200 hover:border-orange-300'
              ...
              <div>
                <label className="text-sm text-gray-600 mb-1 block font-medium">Phone Number</label>
                <input type="tel" value={dataPhone}
                  onChange={e => setDataPhone(e.target.value.replace(/\D/g, '').slice(0, 11))}
                  placeholder="08012345678" className={inputClass} />
              </div>

              <div>
                <label className="text-sm text-gray-600 mb-1 block font-medium">Select Plan</label>
                {loadingPlans ? (
                  ...
                  <div className="text-center py-4 text-gray-400 text-sm">Select a network first</div>
                )}
              </div>

              <button onClick={handleData} disabled={loading || !selectedPlan}
                className="w-full bg-black text-white py-3.5 rounded-xl font-bold hover:bg-gray-800 active:bg-gray-900 disabled:opacity-50 transition-colors text-sm">
          ...
          {activeTab === 'electricity' && (
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <input type="text" value={meterNumber}
                    onChange={e => { setMeterNumber(e.target.value); setMeterName(''); }}
                    placeholder="Enter meter number" className={`flex-1 border border-gray-300 rounded-xl p-3 text-sm focus:outline-none focus:border-orange-500`} />
                  <button onClick={verifyMeter} disabled={verifying || !disco || !meterNumber}
                    className="bg-orange-500 text-white px-4 rounded-xl text-sm font-bold disabled:opacity-50 hover:bg-orange-600 active:bg-orange-700 transition-colors whitespace-nowrap">
                    {verifying ? '...' : 'Verify'}
                  </button>
                </div>
                {meterName && (
              ...
              <div>
                <label className="text-sm text-gray-600 mb-1 block font-medium">Amount (₦)</label>
                <input type="number" value={elecAmount} onChange={e => setElecAmount(e.target.value)}
                  placeholder="Minimum ₦500" className={inputClass} />
                <div className="flex flex-wrap gap-2 mt-2">
                  {[500, 1000, 2000, 5000, 10000, 20000].map(a => (
                    <button key={a} onClick={() => setElecAmount(String(a))}
                      className={`px-3 py-1.5 rounded-lg text-xs font-semibold border transition-colors ${
                        elecAmount === String(a)
                          ? 'bg-orange-500 text-white border-orange-500'
                          : 'bg-gray-100 text-gray-700 border-gray-200 hover:border-orange-400 hover:text-orange-500'
              ...
              <div>
                <label className="text-sm text-gray-600 mb-1 block font-medium">Phone Number</label>
                <input type="tel" value={elecPhone}
                  onChange={e => setElecPhone(e.target.value.replace(/\D/g, '').slice(0, 11))}
                  placeholder="08012345678" className={inputClass} />
              </div>

              <button onClick={handleElectricity} disabled={loading || !meterName}
                className="w-full bg-black text-white py-3.5 rounded-xl font-bold hover:bg-gray-800 active:bg-gray-900 disabled:opacity-50 transition-colors text-sm">
                {loading ? '⏳ Processing...' : `Pay Electricity${elecAmount ? ` — ₦${Number(elecAmount).toLocaleString()}` : ''}`}
              </button>
            </div>
          )}
          ...
          {activeTab === 'cable' && (
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {CABLE_PROVIDERS.map(p => (
                    <button key={p.id}
                      onClick={() => { setCableProvider(p.id); setCardName(''); setSmartCard(''); }}
                      className={`p-3 rounded-xl border-2 text-center transition-all font-bold text-sm ${
                        cableProvider === p.id
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <input type="text" value={smartCard}
                    onChange={e => { setSmartCard(e.target.value); setCardName(''); }}
                    placeholder="Enter smart card number"
                    className="flex-1 border border-gray-300 rounded-xl p-3 text-sm focus:outline-none focus:border-orange-500" />
                  <button onClick={verifySmartCard} disabled={verifying || !cableProvider || !smartCard}
                    className="bg-orange-500 text-white px-4 rounded-xl text-sm font-bold disabled:opacity-50 hover:bg-orange-600 active:bg-orange-700 transition-colors whitespace-nowrap">
                    {verifying ? '...' : 'Verify'}
                  </button>
                </div>
              ...
              <div>
                <label className="text-sm text-gray-600 mb-1 block font-medium">Phone Number</label>
                <input type="tel" value={cablePhone}
                  onChange={e => setCablePhone(e.target.value.replace(/\D/g, '').slice(0, 11))}
                  placeholder="08012345678" className={inputClass} />
              </div>

              <button onClick={handleCable} disabled={loading || !cardName || !selectedCablePlan}
                className="w-full bg-black text-white py-3.5 rounded-xl font-bold hover:bg-gray-800 active:bg-gray-900 disabled:opacity-50 transition-colors text-sm">
                {loading ? '⏳ Processing...' : 'Pay Cable TV'}
              </button>
            </div>
          )}
          ...
          {result?.success && (
            <div className="p-4 bg-orange-50 border border-orange-300 rounded-xl space-y-2">
              ...
              <p className="text-xs text-gray-500">
                Transaction ID: {result.data?.content?.transactions?.transactionId || 'N/A'}
              </p>
          ...
          <p className="text-center text-xs text-gray-400 pb-2">
            Powered by VTpass • <span className="text-orange-500 font-semibold">OBISCO Store</span>
          </p>
