...
export default function VTUPage({ onClose }) {
  ...
  const initiatePaystackPayment = (amount, email, onSuccess) => {
    const handler = window.PaystackPop.setup({
      ...
      onClose: function () { setLoading(false); setError("Payment cancelled."); },
    });
    handler.openIframe();
  };
  ...
  const handleAirtime = () => {
    ...
    if (!email) { setShowEmailInput(true); setPendingAction("airtime"); setPendingAmount(Number(airtimeAmount)); return; }
  ...
  const handleEmailSubmit = () => {
    ...
    initiatePaystackPayment(pendingAmount, customerEmail, async () => {
      ...
      if (pendingAction === "electricity") await handleElectricityAfterPayment();
          ...
          <div className="absolute inset-0 bg-black/50 z-10 flex items-center justify-center rounded-t-2xl sm:rounded-2xl">
            <div className="bg-white mx-4 p-6 rounded-2xl shadow-xl w-full max-w-sm">
              ...
              <input type="email" value={customerEmail} onChange={(e) => setCustomerEmail(e.target.value)}
                placeholder="yourname@gmail.com" className="w-full border rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-orange-500 mb-3" />
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          {activeTab === "airtime" && (
            <div className="space-y-4">
              ...
              <button onClick={handleAirtime} disabled={loading}
                ...
                {loading ? "⏳ Processing..." : `💳 Pay & Get Airtime${airtimeAmount ? ` — ₦${Number(airtimeAmount).toLocaleString()}` : ""}`}
              </button>
            </div>
          )}
          ...
          {activeTab === "electricity" && (
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <input type="text" value={meterNumber} onChange={(e) => { setMeterNumber(e.target.value); setMeterName(""); }}
              ...
              <button onClick={handleElectricity} disabled={loading || !meterName}
                className="w-full bg-black text-white py-3.5 rounded-xl font-bold hover:bg-gray-800 disabled:opacity-50 transition-colors text-sm">
                {loading ? "⏳ Processing..." : `💳 Pay & Get Electricity${elecAmount ? ` — ₦${Number(elecAmount).toLocaleString()}` : ""}`}
              </button>
            </div>
          )}
          ...
          {activeTab === "cable" && (
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {CABLE_PROVIDERS.map((p) => (
                    <button key={p.id} onClick={() => { setCableProvider(p.id); setCardName(""); setSmartCard(""); }}
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <input type="text" value={smartCard} onChange={(e) => { setSmartCard(e.target.value); setCardName(""); }}
                    placeholder="Enter smart card number" className="flex-1 border border-gray-300 rounded-xl p-3 text-sm focus:outline-none focus:border-orange-500" />
              ...
              <div>
                <label className="text-sm text-gray-600 mb-1 block font-medium">Phone Number</label>
                <input type="tel" value={cablePhone} onChange={(e) => setCablePhone(e.target.value.replace(/\D/g, "").slice(0, 11))} placeholder="08012345678" className={inputClass} />
              </div>
          ...
          {result?.success && (
            <div className="p-4 bg-orange-50 border border-orange-300 rounded-xl space-y-2">
              ...
              <button onClick={() => setResult(null)} className="w-full mt-1 bg-orange-500 text-white py-2.5 rounded-lg text-sm font-bold hover:bg-orange-600 transition-colors">
