...
const HOT_PLAN_CODES = [
  ...
  "airt-2500-7",
  "airt-350-500",
  "glo-350-special-1day",
  "glo-1000mb-300-oneoff",
  "glo-500mb-200-oneoff",
  "glo-600-special-2days",
  "glo-1000-special-2days",
  "glo-special-1500",
  "glo-2000-7days",
  "glo-5000-7days",
  "glo-youtube-250",
...
const getSaved = (type) => {
  try {
    return JSON.parse(
      localStorage.getItem(`obisco_beneficiaries_${type}`) || "[]",
    );
  } catch {
    return [];
  }
...
const saveBeneficiary = (type, phone, network, name = "") => {
  const existing = getSaved(type);
  const filtered = existing.filter((b) => b.phone !== phone);
  const updated = [
    { phone, network, name, savedAt: Date.now() },
    ...filtered,
  ].slice(0, 5);
  localStorage.setItem(`obisco_beneficiaries_${type}`, JSON.stringify(updated));
};

const deleteBeneficiary = (type, phone) => {
  const existing = getSaved(type);
  const updated = existing.filter((b) => b.phone !== phone);
  localStorage.setItem(`obisco_beneficiaries_${type}`, JSON.stringify(updated));
};
  ...
  if (saved.length === 0) {
    return (
      <input
        ...
        placeholder=""
        className="w-full border border-gray-200 rounded-2xl p-4 text-sm text-gray-700 focus:outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-100 bg-gray-50"
      />
    );
  }
...
export default function VTUPage({ onClose }) {
  ...
  const [beneficiaryKey, setBeneficiaryKey] = useState(0);
  ...
  useEffect(() => {
    if (dataNetwork) {
      ...
      fetch(`${API_BASE}/api/vtu/variations/${dataNetwork}`)
        .then((r) => r.json())
        .then((d) => {
          if (d.content?.variations) setDataPlans(d.content.variations);
        })
  ...
  useEffect(() => {
    if (cableProvider) {
      ...
      fetch(`${API_BASE}/api/vtu/variations/${cableProvider}`)
        .then((r) => r.json())
        .then((d) => {
          if (d.content?.variations) setCablePlans(d.content.variations);
        })
  ...
  // ── Auto detect network on airtime phone change ──
  const handleAirtimePhoneChange = (val) => {
    ...
    if (detected && cleaned.length >= 4) {
      setAirtimeNetwork(detected);
    }
  };

  // ── Auto detect network on data phone change ──
  ...
  const verifyMeter = async () => {
    ...
    try {
      ...
      if (data.content?.Customer_Name) setMeterName(data.content.Customer_Name);
      else setError("Meter number not found.");
    } catch {
  ...
  const handleAirtime = async () => {
    ...
    const doRequest = async () => {
      const res = await fetch(`${API_BASE}/api/vtu/airtime`, {
        ...
        body: JSON.stringify({
          ...
          phone: airtimePhone,
          amount,
          ...(vtuPaymentMethod === "wallet"
            ? { userId: user._id || user.id, paymentMethod: "wallet" }
            : {}),
        }),
      });
      return res.json();
    ...
    if (vtuPaymentMethod === "wallet") {
      if (!user?._id && !user?.id)
        return setError("Please log in to pay with wallet");
      ...
      try {
        ...
        if (
          ...
          setResult({
            success: true,
            message: `✅ ₦${amount.toLocaleString()} airtime sent to ${airtimePhone} successfully!`,
            data,
          });
          ...
          setAirtimeAmount("");
          setAirtimeNetwork("");
          setDetectedAirtimeNetwork(null);
          setBeneficiaryKey((k) => k + 1);
    ...
    if (vtuPaymentMethod === "wallet") {
      ...
      try {
        ...
        if (
          ...
          setResult({
            success: true,
            message: `✅ ${selectedPlan.name} sent to ${dataPhone} successfully!`,
            data,
          });
          setWalletBalance((prev) => prev - amount);
          setDataPhone("");
    ...
    initiatePaystackPayment(pendingAmount, customerEmail, async () => {
      if (pendingAction === "airtime") {
        ...
        if (
          ...
          setResult({
            success: true,
            message: `✅ ₦${Number(airtimeAmount).toLocaleString()} airtime sent!`,
            data,
          });
          setBeneficiaryKey((k) => k + 1);
        } else setError(data.response_description || "Transaction failed.");
      ...
      if (pendingAction === "data" && selectedPlan) {
        ...
        if (
          ...
          setResult({
            success: true,
            message: `✅ ${selectedPlan.name} sent!`,
            data,
          });
          setBeneficiaryKey((k) => k + 1);
        } else setError(data.response_description || "Transaction failed.");
      ...
      if (pendingAction === "cable") {
        const plan = cablePlans.find(
          (p) => p.variation_code === selectedCablePlan,
        );
        const res = await fetch(`${API_BASE}/api/vtu/cable`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const inputClass =
    "w-full border border-gray-200 rounded-2xl p-4 text-sm text-gray-700 focus:outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-100 bg-gray-50";
  const selectClass =
    "w-full border border-gray-200 rounded-2xl p-4 text-sm text-gray-700 focus:outline-none focus:border-orange-500 bg-gray-50 appearance-none cursor-pointer";
  ...
  const PaymentToggle = ({ amount }) => (
    <div className="space-y-3">
      ...
      <div className="flex gap-3">
        <button
          onClick={() => setVtuPaymentMethod("paystack")}
          className={`flex-1 py-3 rounded-2xl text-sm font-bold border-2 transition flex flex-col items-center gap-1 ${vtuPaymentMethod === "paystack" ? "bg-orange-500 text-white border-orange-500" : "border-gray-200 text-gray-600 bg-white"}`}
        >
          <span>💳 Card</span>
          <span
            className={`text-xs font-normal ${vtuPaymentMethod === "paystack" ? "text-orange-100" : "text-gray-400"}`}
          >
            Paystack
          </span>
        </button>
        <button
          onClick={() => setVtuPaymentMethod("wallet")}
          className={`flex-1 py-3 rounded-2xl text-sm font-bold border-2 transition flex flex-col items-center gap-1 ${vtuPaymentMethod === "wallet" ? "bg-orange-500 text-white border-orange-500" : "border-gray-200 text-gray-600 bg-white"}`}
        >
          <span>💰 Wallet</span>
          <span
            className={`text-xs font-normal ${vtuPaymentMethod === "wallet" ? "text-orange-100" : "text-gray-500"}`}
          >
      ...
      {vtuPaymentMethod === "wallet" && amount > 0 && (
        <div
          className={`rounded-2xl p-3 text-xs font-medium ${walletBalance >= amount ? "bg-green-50 text-green-700" : "bg-red-50 text-red-600"}`}
        >
          {walletBalance >= amount
            ? `✅ Sufficient balance`
            : `❌ You need ₦${(amount - walletBalance).toLocaleString()} more`}
        </div>
      )}
    </div>
  );
  ...
  const PlanBottomSheet = () => {
    ...
    const filteredPlans =
      planFilter === "Hot" ? grouped.Hot : grouped[planFilter] || [];
    return (
      <div
      ...
      >
        <div
          className="bg-white w-full sm:max-w-lg rounded-t-3xl flex flex-col"
        ...
        >
          <div className="p-5 border-b flex justify-between items-center flex-shrink-0">
            ...
            <button
              onClick={() => setShowPlanSheet(false)}
              className="w-9 h-9 bg-gray-100 rounded-full flex items-center justify-center text-gray-500 font-bold text-lg"
            >
              ✕
            </button>
          </div>
          <div className="flex gap-2 p-4 overflow-x-auto flex-shrink-0 border-b">
            {filters.map((f) => (
              <button
                key={f}
                onClick={() => setPlanFilter(f)}
                className={`px-4 py-2 rounded-full text-xs font-bold whitespace-nowrap flex-shrink-0 transition-all ${planFilter === f ? (f === "Hot" ? "bg-red-500 text-white" : "bg-orange-500 text-white") : "bg-gray-100 text-gray-600"}`}
              >
                {f === "Hot" ? "🔥 Hot" : f}
              </button>
            ))}
              ...
              filteredPlans.map((plan) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left p-4 rounded-2xl border-2 transition-all ${isSelected ? "border-orange-500 bg-orange-50" : "border-gray-100 bg-gray-50 hover:border-orange-200"}`}
                  >
                    <div className="flex justify-between items-center gap-2">
                      <p
                        className={`text-sm font-bold leading-tight flex-1 ${isSelected ? "text-orange-700" : "text-gray-800"}`}
                      >
                        {plan.name}
                      </p>
                      <div className="text-right flex-shrink-0">
                        <p
                          className={`text-base font-black ${isSelected ? "text-orange-600" : "text-gray-900"}`}
                        >
  ...
  const SuccessCard = ({ message, transactionId }) => (
    <div className="flex flex-col items-center justify-center py-12 px-6 text-center">
      ...
      <button
        ...
        className="w-full bg-orange-500 text-white py-4 rounded-2xl font-bold text-sm hover:bg-orange-600 transition mb-3"
      >
        Make Another Payment
      </button>
      <button
        onClick={onClose}
        className="w-full border border-gray-200 text-gray-600 py-4 rounded-2xl font-bold text-sm hover:bg-gray-50 transition"
      >
        Back to Store
      </button>
    </div>
  ...
  const PageHeader = ({ title, onBack }) => (
    <div className="flex items-center gap-4 p-5 border-b bg-white sticky top-0 z-10 flex-shrink-0">
      <button
        onClick={onBack}
        className="w-10 h-10 bg-gray-100 rounded-full flex items-center justify-center text-gray-600 hover:bg-gray-200 transition font-bold text-lg"
      >
        ←
      </button>
  ...
  const EmailModal = () => (
    <div className="fixed inset-0 bg-black/50 z-[300] flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl p-6 w-full max-w-sm shadow-2xl">
        <h3 className="font-black text-gray-800 mb-1 text-lg">
          Enter your email
        </h3>
        ...
        <input
          ...
          className="w-full border border-gray-200 rounded-2xl px-4 py-3 text-sm focus:outline-none focus:border-orange-500 mb-4 bg-gray-50"
        />
        <div className="flex gap-3">
          <button
            onClick={() => setShowEmailInput(false)}
            className="flex-1 border border-gray-200 text-gray-500 font-bold py-3 rounded-2xl text-sm"
          >
            Cancel
          </button>
          <button
            onClick={handleEmailSubmit}
            className="flex-1 bg-orange-500 text-white font-bold py-3 rounded-2xl text-sm hover:bg-orange-600"
          >
            Continue
          </button>
        </div>
  ...
  const NetworkDetectedBadge = ({ networkId }) => {
    ...
    return (
      <div className="flex items-center gap-2 mt-2 px-3 py-2 bg-green-50 border border-green-200 rounded-xl">
        <div
          className={`w-5 h-5 rounded-full ${networkBg(networkId)} flex items-center justify-center flex-shrink-0`}
        >
          <span className={`text-xs font-black ${networkText(networkId)}`}>
            {networkLabel(networkId)[0]}
          </span>
        </div>
        <p className="text-xs text-green-700 font-bold">
          ✓ {networkLabel(networkId)} detected automatically
        </p>
      </div>
    );
  };
        ...
        <div className="flex flex-col h-full">
          <div className="bg-orange-500 px-5 pt-12 pb-8 flex-shrink-0">
            <div className="flex justify-between items-center mb-6">
              <button
                onClick={onClose}
                className="w-10 h-10 bg-orange-400 rounded-full flex items-center justify-center text-white font-bold text-lg hover:bg-orange-600 transition"
              >
                ←
              </button>
            ...
            <div className="grid grid-cols-2 gap-4">
              {SERVICES.map((service) => (
                <button
                  ...
                  className={`p-5 rounded-3xl border-2 text-left transition-all hover:shadow-md active:scale-95 ${service.color}`}
                >
              ...
              <>
                <div>
                  ...
                  <BeneficiaryList
                    key={beneficiaryKey}
                    type="airtime"
                    currentPhone={airtimePhone}
                    onChange={handleAirtimePhoneChange}
                    onSelect={(b) => {
                      setAirtimePhone(b.phone);
                      setAirtimeNetwork(b.network);
                      setDetectedAirtimeNetwork(b.network);
                    }}
                    onDelete={() => setBeneficiaryKey((k) => k + 1)}
                  />
                ...
                <div>
                  <label className="text-sm font-bold text-gray-700 mb-3 block">
                    Network{" "}
                    {detectedAirtimeNetwork && (
                      <span className="text-orange-500 font-normal text-xs ml-1">
                        (auto-selected)
                      </span>
                    )}
                  </label>
                  <div className="grid grid-cols-4 gap-3">
                    {NETWORKS.map((n) => (
                      <button
                        key={n.id}
                        onClick={() => {
                          setAirtimeNetwork(n.id);
                          setDetectedAirtimeNetwork(null);
                        }}
                        className={`p-3 rounded-2xl border-2 text-center transition-all ${airtimeNetwork === n.id ? "border-orange-500 shadow-md bg-orange-50" : "border-gray-200 bg-gray-50 hover:border-orange-300"}`}
                      >
                        <div
                          className={`w-10 h-10 rounded-full ${n.bg} mx-auto flex items-center justify-center mb-1`}
                        >
                  ...
                  <div className="flex flex-wrap gap-2 mt-3">
                    {quickAmounts.map((a) => (
                      <button
                        ...
                        className={`px-4 py-2 rounded-xl text-xs font-bold border transition-all ${airtimeAmount === String(a) ? "bg-orange-500 text-white border-orange-500" : "bg-gray-100 text-gray-700 border-gray-200 hover:border-orange-400"}`}
                      >
                        ₦{a.toLocaleString()}
                      </button>
                    ))}
                ...
                <button
                  ...
                  disabled={loading}
                  className="w-full bg-orange-500 text-white py-4 rounded-2xl font-black hover:bg-orange-600 disabled:opacity-50 transition text-sm"
                >
                  {loading
                    ? "⏳ Processing..."
                    : `${vtuPaymentMethod === "wallet" ? "💰" : "💳"} Pay${airtimeAmount ? ` ₦${Number(airtimeAmount).toLocaleString()}` : ""}`}
                </button>
              </>
            )}
          </div>
              ...
              <>
                <div>
                  ...
                  <BeneficiaryList
                    key={beneficiaryKey}
                    type="data"
                    currentPhone={dataPhone}
                    onChange={handleDataPhoneChange}
                    onSelect={(b) => {
                      setDataPhone(b.phone);
                      setDataNetwork(b.network);
                      setDetectedDataNetwork(b.network);
                      setSelectedPlan(null);
                      setPlanFilter("Hot");
                    }}
                    onDelete={() => setBeneficiaryKey((k) => k + 1)}
                  ...
                  <div className="grid grid-cols-4 gap-3">
                    {DATA_NETWORKS.map((n) => (
                      <button
                        ...
                        className={`p-3 rounded-2xl border-2 text-center transition-all ${dataNetwork === n.id ? "border-orange-500 shadow-md bg-orange-50" : "border-gray-200 bg-gray-50 hover:border-orange-300"}`}
                      >
                        <div
                          className={`w-10 h-10 rounded-full ${n.bg} mx-auto flex items-center justify-center mb-1`}
                        >
                ...
                >
                  {loading
                    ? "⏳ Processing..."
                    : `${vtuPaymentMethod === "wallet" ? "💰" : "💳"} Pay${selectedPlan ? ` ₦${Number(selectedPlan.variation_amount).toLocaleString()}` : ""}`}
                </button>
              </>
            )}
          </div>
            ...
            {result?.success ? (
              ...
              <>
                  ...
                  <div className="flex gap-3">
                    {["prepaid", "postpaid"].map((type) => (
                      <button
                        ...
                        className={`flex-1 py-3 rounded-2xl border-2 text-sm font-bold capitalize transition-all ${meterType === type ? "border-orange-500 bg-orange-50 text-orange-600" : "border-gray-200 bg-gray-50 text-gray-600"}`}
                      >
                        {type}
                      </button>
                    ))}
                ...
                <div>
                  ...
                  <div className="flex gap-3">
                    <input
                      ...
                      className="flex-1 border border-gray-200 rounded-2xl p-4 text-sm focus:outline-none focus:border-orange-500 bg-gray-50"
                    />
                    <button
                      onClick={verifyMeter}
                      disabled={verifying || !disco || !meterNumber}
                      className="bg-orange-500 text-white px-5 rounded-2xl text-sm font-bold disabled:opacity-50 hover:bg-orange-600 transition whitespace-nowrap"
                    >
                      {verifying ? "..." : "Verify"}
                    </button>
                  </div>
                  {meterName && (
                    <div className="mt-3 p-3 bg-green-50 rounded-2xl text-sm text-green-700 font-bold">
                      ✅ {meterName}
                    </div>
                  )}
                </div>
                  ...
                  <div className="flex flex-wrap gap-2 mt-3">
                    {[500, 1000, 2000, 5000, 10000, 20000].map((a) => (
                      <button
                        ...
                        className={`px-4 py-2 rounded-xl text-xs font-bold border transition-all ${elecAmount === String(a) ? "bg-orange-500 text-white border-orange-500" : "bg-gray-100 text-gray-700 border-gray-200 hover:border-orange-400"}`}
                      >
                        ₦{a.toLocaleString()}
                      </button>
                    ))}
                ...
                >
                  {loading
                    ? "⏳ Processing..."
                    : `${vtuPaymentMethod === "wallet" ? "💰" : "💳"} Pay${elecAmount ? ` ₦${Number(elecAmount).toLocaleString()}` : ""}`}
                </button>
              </>
            )}
          </div>
            ...
            {result?.success ? (
              ...
              <>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {CABLE_PROVIDERS.map((p) => (
                      <button
                        ...
                        className={`p-4 rounded-2xl border-2 text-center transition-all ${cableProvider === p.id ? "border-orange-500 bg-orange-50 shadow-md" : "border-gray-200 bg-gray-50 hover:border-orange-300"}`}
                      >
                ...
                <div>
                  ...
                  {loadingCablePlans ? (
                    <div className="p-5 text-center text-orange-500 text-sm font-bold">
                      ⏳ Loading plans...
                    </div>
                  ) : cablePlans.length > 0 ? (
                    <div className="relative">
                ...
                >
                  {loading
                    ? "⏳ Processing..."
                    : `${vtuPaymentMethod === "wallet" ? "💰" : "💳"} Pay & Activate`}
                </button>
              </>
            )}
          </div>
