    ...
    <>
      ...
      <div className="fixed inset-0 z-[70] flex flex-col bg-white">
        ...
        <div className="flex border-b bg-orange-50 shrink-0">
          {[
            { id: "products", label: "📦 My Products", icon: <FaBox size={14} /> },
        ...
        <div className="flex-1 overflow-y-auto p-4">
          ...
          {success && (
            <div className="mb-4 p-3 bg-green-50 border border-green-200 rounded-xl text-sm text-green-600">✅ {success}</div>
          )}
          ...
          {activeTab === "products" && (
            <div>
              ...
              {!showForm && (
                <button
                  onClick={() => { setShowForm(true); setForm(emptyForm); setEditingId(null); setError(""); setSuccess(""); }}
                ...
                <div className="bg-orange-50 border border-orange-200 rounded-2xl p-4 mb-4">
                  ...
                  <div className="flex flex-col gap-3">
                    <div>
                      ...
                      <input name="name" value={form.name} onChange={handleChange} placeholder="e.g iPhone 15 Pro Max"
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div>
                        ...
                        <input name="category" value={form.category} onChange={handleChange} placeholder="e.g Phones"
                          className="w-full border rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-orange-500 bg-white" />
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <input name="image" value={form.image} onChange={handleChange} placeholder="https://example.com/image.jpg"
                        className="w-full border rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-orange-500 bg-white" />
                      {form.image && (
                        <img src={form.image} alt="preview" className="mt-2 h-20 w-20 object-cover rounded-xl border" onError={(e) => e.target.style.display = 'none'} />
                      )}
                    </div>

                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="text-xs font-semibold text-gray-600 mb-1 block">Price Range *</label>
                      ...
                      <div>
                        ...
                        <input name="amount" type="number" value={form.amount} onChange={handleChange} placeholder="e.g 50000"
                          className="w-full border rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-orange-500 bg-white" />
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      <input type="checkbox" name="inStock" checked={form.inStock} onChange={handleChange} id="inStock" className="w-4 h-4 accent-orange-500" />
                      <label htmlFor="inStock" className="text-sm text-gray-700 font-medium">In Stock</label>
                    </div>
              ...
              {/* Products List */}
              {loading ? (
                <div className="text-center py-10 text-orange-500 font-medium">⏳ Loading products...</div>
              ) : products.length === 0 ? (
                <div className="text-center py-10">
                  <p className="text-4xl mb-3">📦</p>
                  <p className="text-gray-500 font-medium">No products yet</p>
                  <p className="text-gray-400 text-sm mt-1">Upload your first product above!</p>
                </div>
              ) : (
                <div className="flex flex-col gap-3">
                  {products.map((product) => (
                    <div key={product._id} className="flex items-center gap-3 bg-white border border-gray-200 rounded-2xl p-3 shadow-sm">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-orange-500 text-xs font-semibold">₦{Number(product.amount).toLocaleString()}</p>
                        <div className="flex items-center gap-2 mt-1">
                          <span className="text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full capitalize">{product.department}</span>
                          <span className={`text-xs px-2 py-0.5 rounded-full font-semibold ${product.inStock ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-500'}`}>
            ...
            <div className="flex flex-col gap-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-orange-50 border border-orange-200 rounded-2xl p-4 text-center">
                  ...
                  <p className="text-xs text-gray-500 font-medium mt-1">Total Products</p>
                </div>
                <div className="bg-green-50 border border-green-200 rounded-2xl p-4 text-center">
                  <p className="text-3xl font-black text-green-500">{products.filter(p => p.inStock).length}</p>
                  <p className="text-xs text-gray-500 font-medium mt-1">In Stock</p>
                </div>
              </div>

              <div className="bg-white border border-gray-200 rounded-2xl p-4">
                <h3 className="font-bold text-gray-800 text-sm mb-3">📋 Business Info</h3>
                <div className="flex flex-col gap-2">
                  <p className="text-sm text-gray-600"><span className="font-semibold">Business:</span> {seller?.businessName}</p>
                  <p className="text-sm text-gray-600"><span className="font-semibold">Category:</span> {seller?.businessCategory}</p>
                  <p className="text-sm text-gray-600"><span className="font-semibold">Email:</span> {seller?.email}</p>
                </div>
              </div>

              <div className="bg-yellow-50 border border-yellow-200 rounded-2xl p-4">
                <p className="text-xs font-bold text-yellow-700 mb-1">📊 More Stats Coming Soon</p>
                <p className="text-xs text-yellow-600">Order statistics and earnings will be available soon!</p>
              </div>
            </div>
          )}
        </div>
