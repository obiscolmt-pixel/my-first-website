...
import {
  FaMobileAlt, FaTshirt, FaStar, FaBolt, FaWallet,
...
const CATEGORIES = [
  { label: "Gadgets", icon: <FaMobileAlt size={22} />, dept: "gadgets", color: "bg-blue-100 text-blue-600" },
  { label: "Fashion", icon: <FaTshirt size={22} />, dept: "fashion", color: "bg-pink-100 text-pink-600" },
  { label: "Lifestyle", icon: <FaStar size={22} />, dept: "lifestyle", color: "bg-purple-100 text-purple-600" },
  { label: "Pay Bills", icon: <FaBolt size={22} />, dept: "vtu", color: "bg-yellow-100 text-yellow-600" },
  { label: "Wallet", icon: <FaWallet size={22} />, dept: "wallet", color: "bg-green-100 text-green-600" },
];

const StarDisplay = ({ rating, size = "text-sm" }) => (
  <div className="flex">
    {[1, 2, 3, 4, 5].map((star) => (
      <span key={star} className={`${size} ${star <= Math.round(rating) ? "text-yellow-400" : "text-gray-200"}`}>★</span>
    ))}
  </div>
);

export default function HomePage({
  addToCart, addToWishlist, isWishlisted,
  setActiveDepartment, setShowVTU, setWalletOpen,
}) {
  ...
  const handleBannerCta = (dept) => {
    if (dept === "vtu") { setShowVTU(true); return; }
    if (dept === "wallet") { setWalletOpen(true); return; }
  ...
  const handleSubmitReview = async () => {
    ...
    if (!userComment.trim()) { alert("Please write a comment."); return; }
    setReviewLoading(true);
    const res = await addReview(selectedProduct._id, {
      rating: userRating, comment: userComment, fullName: user.fullName,
    });
  ...
  const lifestyle = featuredProducts.filter((p) => p.department === "lifestyle");
  ...
  const ProductCard = ({ item }) => (
    <div className="shadow-md rounded-xl hover:scale-105 duration-300 overflow-hidden bg-white cursor-pointer shrink-0 w-[160px] sm:w-[200px]">
      ...
      <div className="px-2 pt-2">
        ...
        <p className="text-orange-500 font-black text-xs mt-0.5">₦{item.amount.toLocaleString()}</p>
    ...
    return (
      <div className="mt-8">
        <div className="flex items-center justify-between mb-3">
          <h2 className="font-black text-lg sm:text-xl text-gray-800">{emoji} {title}</h2>
          <button
  ...
  return (
    <div className="max-w-[1640px] mx-auto px-3 sm:px-4 pb-8">
      ...
      {toast && (
        <div className="fixed top-4 left-0 right-0 flex justify-center z-50 animate-slideDown">
          <div className="flex items-center gap-2 bg-green-400 shadow-2xl px-4 py-2.5 rounded-3xl">
            <img src={toast.image} alt={toast.name} className="w-6 h-6 object-contain rounded-full bg-orange-50 shrink-0" />
      ...
      {selectedProduct && (
        <>
          <div className="fixed inset-0 bg-black/60 z-40" onClick={closeProduct} />
          <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[88%] sm:w-full sm:max-w-3xl bg-white z-50 rounded-2xl shadow-2xl overflow-hidden max-h-[65vh] sm:max-h-[90vh] flex flex-col">
            <button onClick={closeProduct} className="absolute top-3 right-3 sm:top-4 sm:right-4 text-gray-400 hover:text-black z-10 bg-white rounded-full p-1.5 shadow">
            ...
            <div className="flex flex-col sm:flex-row overflow-y-auto">
              <div className="w-full sm:w-1/2 h-[180px] sm:h-auto shrink-0 bg-gray-50 flex items-center justify-center overflow-hidden">
                <img src={selectedImage || selectedProduct.image} alt={selectedProduct.name} className="w-full h-full object-contain p-3 transition-all duration-300" />
              </div>
              <div className="w-full sm:w-1/2 p-3 sm:p-6 flex flex-col gap-2 sm:gap-4 overflow-y-auto">
                <div>
                  <p className="text-orange-500 uppercase tracking-widest text-[10px] font-semibold capitalize">{selectedProduct.category}</p>
                  <h2 className="text-base sm:text-2xl font-bold text-gray-800 mt-1">{selectedProduct.name}</h2>
                </div>
                <div className="flex items-center justify-between flex-wrap gap-2">
                  <span className="text-xl sm:text-3xl font-black text-orange-500">₦{selectedProduct.amount.toLocaleString()}</span>
                  <div className="flex items-center gap-1">
                    <StarDisplay rating={avgRating} />
                    <span className="text-xs text-gray-500">{avgRating} ({totalReviews})</span>
                  </div>
                </div>
                <span className="bg-green-100 text-green-600 text-xs px-2 py-1 rounded-full font-semibold w-fit">In Stock</span>
                <p className="text-gray-400 text-[11px] sm:text-sm leading-relaxed">
                  Premium quality {selectedProduct.name} with the latest features. Comes with full warranty and official accessories. Fast delivery across Nigeria.
                </p>
                <div>
                  <p className="font-semibold text-gray-700 text-xs mb-2">Quantity</p>
                  <div className="flex items-center gap-3 flex-wrap">
                    <button onClick={() => setQuantity((q) => Math.max(1, q - 1))} className="w-9 h-9 rounded-full border border-gray-300 flex items-center justify-center text-xl font-bold hover:bg-gray-100 transition">−</button>
                    <span className="font-black text-lg w-6 text-center">{quantity}</span>
                    <button onClick={() => setQuantity((q) => q + 1)} className="w-9 h-9 rounded-full border border-gray-300 flex items-center justify-center text-xl font-bold hover:bg-gray-100 transition">+</button>
                    <span className="text-gray-400 text-xs sm:text-sm">Subtotal: <span className="text-orange-500 font-bold">₦{(selectedProduct.amount * quantity).toLocaleString()}</span></span>
                  </div>
                </div>
                <div className="bg-orange-50 border border-orange-100 rounded-xl p-2">
                  ...
                  <p className="text-xs text-gray-500">Fast delivery available across Nigeria. Order today and receive within 2-5 business days.</p>
                </div>
                <button
                  onClick={() => { handleAddToCart(selectedProduct, quantity, selectedColor); closeProduct(); }}
                ...
                >
                  <BsFillCartFill size={16} /> Add to Cart — ₦{(selectedProduct.amount * quantity).toLocaleString()}
                </button>
                <button
                ...
                >
                  <MdFavorite size={16} />
                  {isWishlisted(selectedProduct._id) ? "Remove from Wishlist" : "Add to Wishlist"}
                </button>
                <div className="border-t pt-4">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <button onClick={() => setShowReviewForm(!showReviewForm)} className="text-xs text-orange-500 font-bold hover:underline">
                  ...
                  {showReviewForm && (
                    <div className="bg-orange-50 rounded-xl p-3 mb-3 border border-orange-100">
                      ...
                      <div className="flex gap-1 mb-2">
                        {[1, 2, 3, 4, 5].map((star) => (
                          <button key={star} onClick={() => setUserRating(star)} className={`text-2xl transition ${star <= userRating ? "text-yellow-400" : "text-gray-300 hover:text-yellow-300"}`}>★</button>
                        ))}
                      </div>
                      <textarea value={userComment} onChange={(e) => setUserComment(e.target.value)} placeholder="Share your experience..." rows={3} className="w-full border rounded-xl px-3 py-2 text-xs focus:outline-none focus:border-orange-500 resize-none" />
                      <button onClick={handleSubmitReview} disabled={reviewLoading} className="w-full mt-2 bg-orange-500 hover:bg-orange-600 disabled:bg-orange-300 text-white text-xs font-bold py-2 rounded-full transition flex items-center justify-center gap-2">
                        {reviewLoading ? <div className="w-3 h-3 border-2 border-white border-t-transparent rounded-full animate-spin" /> : "Submit Review ⭐"}
                      </button>
                    </div>
                  )}
                  {reviews.length === 0 ? (
                        ...
                        <div key={review._id} className="border rounded-xl p-3 bg-gray-50">
                          <div className="flex justify-between items-start mb-1">
                            <div>
                              ...
                              <p className="text-gray-300 text-xs">{new Date(review.createdAt).toLocaleDateString("en-NG", { day: "numeric", month: "short", year: "numeric" })}</p>
                            </div>
                            <StarDisplay rating={review.rating} size="text-xs" />
                          </div>
                          <p className="text-gray-600 text-xs leading-relaxed mt-1">{review.comment}</p>
                        </div>
                      ))}
                    </div>
                  )}
          ...
          {BANNERS.map((banner) => (
            ...
            >
              <div className="flex-1">
                <p className="text-white/80 text-xs font-semibold uppercase tracking-widest mb-1">Obisco Store</p>
                <h2 className="text-white font-black text-xl sm:text-3xl leading-tight mb-2">{banner.title}</h2>
                <p className="text-white/80 text-xs sm:text-sm mb-4">{banner.subtitle}</p>
                <button
              ...
              <span className="text-6xl sm:text-8xl ml-4 hidden sm:block">{banner.emoji}</span>
            </div>
          ))}
        </div>
        <button
      ...
      <div className="mt-6 grid grid-cols-5 gap-2 sm:gap-4">
        {CATEGORIES.map((cat) => (
          ...
          >
            <div className={`w-12 h-12 sm:w-16 sm:h-16 rounded-2xl ${cat.color} flex items-center justify-center group-hover:scale-110 transition`}>
              {cat.icon}
            </div>
            <span className="text-[10px] sm:text-xs font-semibold text-gray-600 text-center">{cat.label}</span>
          </button>
        ))}
      </div>
      ...
      {!loading && (
        <>
          ...
          <SectionRow title="Featured Gadgets" emoji="📱" products={gadgets} dept="gadgets" />
          <SectionRow title="Featured Fashion" emoji="👗" products={fashion} dept="fashion" />
          <SectionRow title="Featured Lifestyle" emoji="✨" products={lifestyle} dept="lifestyle" />
          ...
          >
            <div>
              <p className="text-orange-500 text-xs font-bold uppercase tracking-widest mb-1">Pay Bills</p>
              <h3 className="text-white font-black text-lg sm:text-2xl mb-1">Airtime • Data • Light • Cable</h3>
              <p className="text-gray-400 text-xs sm:text-sm">All your bills in one place — fast and secure</p>
            </div>
          ...
          {hotDeals.length > 0 && (
            <div className="mt-8">
              <div className="mb-3">
                <h2 className="font-black text-lg sm:text-xl text-gray-800">🔥 Hot Deals</h2>
                <p className="text-xs text-gray-400 mt-0.5">Limited time offers — grab them fast!</p>
              </div>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {hotDeals.map((item) => (
                  <div key={item._id} className="shadow-md rounded-xl overflow-hidden bg-white cursor-pointer hover:scale-105 duration-300">
                    <div onClick={() => openProduct(item)} className="relative">
                      <div className="w-full h-[140px] bg-gray-50 flex items-center justify-center overflow-hidden">
                        <img src={item.image} alt={item.name} className="w-full h-full object-contain p-2" />
                      </div>
          ...
          {featuredProducts.length === 0 && hotDeals.length === 0 && (
            <div className="text-center py-16">
              ...
              <p className="text-gray-500 text-base font-semibold">No featured products yet</p>
              <p className="text-gray-400 text-sm mt-1">Add featured products from your admin dashboard</p>
            </div>
          )}
        </>
      )}
